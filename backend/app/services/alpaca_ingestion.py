...
# Delay tra richieste consecutive (Alpaca free: 200 req/min → 0.3s safe)
_REQUEST_DELAY_S: float = 0.35
# Upsert chunk size (stesso di Yahoo Finance)
_UPSERT_CHUNK_SIZE: int = 500
        ...
        async with httpx.AsyncClient(headers=headers, timeout=30.0) as client:
            for symbol in symbols:
                ...
                for tf in timeframes:
                    ...
                    if len(rows) >= 5000:
                        await _chunked_upsert_candles(session, rows)
                        rows = []
